...
public class TaskC {
  public void solve(int testNumber, InputReader in, OutputWriter out) {
    ...
    for (int i = 1; i < n; i++) {
      SegmentTree segmentTreeRMQ = new SegmentTree(dp[i - 1]);
      for (int j = 1; j < N; j++) {
        char x = s.charAt(i - 1);
        if (x == '=') {
          dp[i][j] = dp[i - 1][j] + j;
        } else if (x == '>') {
          dp[i][j] = segmentTreeRMQ.query(1, j + 1, N - 1) + j;
        } else {
          dp[i][j] = segmentTreeRMQ.query(1, 1, j - 1) + j;
        }
      }
    }
    int j = n - 1;
